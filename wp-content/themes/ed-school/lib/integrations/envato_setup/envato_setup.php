...
if ( ! class_exists( 'Envato_Theme_Setup_Wizard' ) ) {
	...
	class Envato_Theme_Setup_Wizard {
		...
		protected $site_style_theme_mod_key = 'ed_school_site_style';
		...
		public function init_actions() {
			if ( apply_filters( $this->theme_name . '_enable_setup_wizard', true ) && current_user_can( 'manage_options' ) ) {
				...
				add_filter( 'ed-school-tgmpa-filter-plugins', array( $this, 'filter_plugins' ) );
			}
		...
		public function redux_panel_will_appear_here() {
			...
			<div class="notice notice-warning"> 
				<p><strong><?php esc_html_e( 'Redux panel will appear here.', 'ed-school' ); ?></strong></p>
			</div>
			<?php
		}
		...
		public function admin_menus() {
			if ( ! class_exists( 'Redux' ) ) {
				...
				add_menu_page(
			        esc_html__( 'Theme Options', 'ed-school' ),
			        esc_html__( 'Theme Options', 'ed-school' ),
			        'manage_options',
			        $this->parent_slug,
			        array(
			...
			<?php
				...
				if ( $image_url ) {
					...
					<img src="<?php echo esc_url( $this->plugin_url . 'images/logo.png' ); ?>"
					     alt="<?php esc_attr_e( 'Envato install wizard', 'ed-school' ); ?>" /><?php
				} ?>
		</h1>
		<?php
		}
	...
	<?php
		...
		public function envato_setup_requirements() {
			...
			<?php if ( $this->installation_video_link ): ?>
				<p>
					<a target="_blank" href="<?php echo esc_url( $this->installation_video_link ); ?>"><img
							...
							alt="<?php esc_attr_e( 'Installation video', 'ed-school' ); ?>"/></a>
				</p>
			<?php endif ?>
			...
			<ul>
				<li><?php esc_html_e( 'WordPress 5.0 or higher', 'ed-school' ); ?></li>
				<li><?php esc_html_e( 'PHP 7.4 or higher', 'ed-school' ); ?></li>
			...
			<?php esc_html_e( 'WPBakery Page Builder and Revolution Slider plugins are included with the theme but you do not get purchase codes for these plugins.', 'ed-school' ); ?>
			</h4>
			<p>
			...
			<?php if ( class_exists( 'WooCommerce' ) ) : ?>
				<br/>
				<p>
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=wc-status' ) ); ?>"><?php esc_html_e( 'Check System Status page', 'ed-school' ); ?></a>
				</p>
			<?php endif; ?>
		...
		private function _get_plugins() {
			if ( function_exists( 'ed_school_setup_wizard_get_plugins' ) ) {
				return ed_school_setup_wizard_get_plugins();
			} else {
							...
							<li data-slug="<?php echo esc_attr( $slug ); ?>"><?php echo esc_html( $plugin['name'] ); ?>
								<span>
								    ...
								    if ( isset( $plugins['install'][ $slug ] ) ) {
									    $keys[] = esc_html_e( 'Installation', 'ed-school' );
								    }
								    if ( isset( $plugins['update'][ $slug ] ) ) {
									    $keys[] = esc_html_e( 'Update', 'ed-school' );
								    }
								    if ( isset( $plugins['activate'][ $slug ] ) ) {
									    $keys[] = esc_html_e( 'Activation', 'ed-school' );
								    }
								    printf( esc_html__( '%s required', 'ed-school' ), implode( ' and ', $keys ) ); 
								    ?>
    							</span>
		...
		public function envato_setup_rev_slider() {
			...
			if ( $use_rev_slider ) {
				...
				$rev_importer = new Ed_School_Revolution_Slider_Importer();
				...
				if ( class_exists( 'ZipArchive' ) ) {
					...
					$ls_importer = new Ed_School_Layer_Slider_Importer();
					$ls_importer->cleanup();
				} else {
					$error = esc_html__('Layer Slider requires PHP ZipArchive extension .zip files. Please contact your hosting provider to enable this for you', 'ed-school');
				}
			}
			?>
			<form method="post" enctype="multipart/form-data">
				...
				<?php else: ?>
					<p><?php echo sprintf( esc_html__( 'Please select %s slider to import', 'ed-school' ), $slider_name ); ?></p>
		...
		public function envato_setup_rev_slider_handler() {
			...
			if ($use_rev_slider) {
				...
				require_once( get_template_directory() . '/lib/integrations/demo-importer/includes/file-importer.php' );
				$ls_importer = new Ed_School_File_Importer();
		...
		public function _content_install_settings() {
			...
			$layout_blocks = ed_school_get_registered_layout_blocks();
		...
		public function envato_setup_updates() {
			...
			<?php if ( function_exists( 'envato_market' ) ) { ?>
				<form method="post">
					...
					if ( count( $my_items ) ) {
						?>
						<p><?php esc_html_e( 'Thanks! Theme updates have been enabled for the following items:', 'ed-school' ); ?></p>
						<p><?php esc_html_e( 'Theme Updates', 'ed-school' ); ?></p>
						<ul>
							<?php foreach ( $my_items as $item ) { ?>
								<li><?php echo esc_html( $item['name'] ); ?></li>
							<?php } ?>
						</ul>
						<p><?php esc_html_e( 'When an update becomes available it will show in the Dashboard with an option to install.', 'ed-school' ); ?></p>
						<p><?php esc_html_e( "Change settings from the 'Envato Market' menu in the WordPress Dashboard.", 'ed-school' ); ?></p>
					...
					<?php
						...
						<p><?php esc_html_e( 'When an update becomes available it will show in the Dashboard with an option to install.', 'ed-school' ); ?></p>
						<p>
							<em><?php printf( esc_html__( 'On the next page you will be asked to Login with your ThemeForest account and grant permissions to enable Automatic Updates. If you have any questions please %s.', 'ed-school' ),
								sprintf( '<a href="%s" target="_blank">%s</a>',
									esc_url( 'https://themeforest.net/user/aislin' ),
									esc_html__( 'contact us', 'ed-school' ) ) ); ?></em>
						</p>
						<p class="envato-setup-actions step">
							<input type="submit" class="button-primary button button-large button-next"
			...
			<?php } else { ?>
				<?php esc_html_e( 'Please ensure the Envato Market plugin has been installed correctly.', 'ed-school' ); ?>
				<a href="<?php echo esc_url( $this->get_step_link( 'default_plugins' ) ); ?>"><?php esc_html_e( 'Return to Required Plugins installer', 'ed-school' ); ?></a>
					<p class="envato-setup-actions step">
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'ed-school' ); ?></a>
					</p>
		...
		public function envato_setup_customize() {
			...
			<h1><?php esc_html_e( 'Theme Customization', 'ed-school' ); ?></h1>
			<p><?php esc_html_e( 'Most changes to the website can be made through the Theme Options dashboard.', 'ed-school' ); ?></p>
			<p><?php esc_html_e( 'To change the Sidebars go to Appearance > Widgets. Here widgets can be "drag &amp; droped" into sidebars.', 'ed-school' ); ?></p>
			<p>
				<em><?php printf( esc_html__( 'Advanced Users: If you are going to make changes to the theme source code please use a %s rather than modifying the main theme HTML/CSS/PHP code. This allows the parent theme to receive updates without overwriting your source code changes.', 'ed-school' ),
					sprintf( '<a href="%s" target="_blank">%s</a>',
					esc_url( 'https://codex.wordpress.org/Child_Themes' ),
					esc_html__( 'Child Theme', 'ed-school' ) ) ); ?>
					<br>
					<?php printf( esc_html__( 'See %s in the main folder for a sample.', 'ed-school' ),
					sprintf( '<code>%s</code>',
					esc_html__( 'child-theme.zip', 'ed-school' ) ) ) ?>
					</em>
			</p>
		...
		public function envato_setup_help_support() {
			?>
			<h1><?php esc_html_e( 'Help and Support', 'ed-school' ); ?></h1>
			<p><?php esc_html_e( 'This theme comes with 6 months item support from purchase date (with the option to extend this period). This license allows you to use this theme on a single website. Please purchase an additional license to use this theme on another website.', 'ed-school' ); ?></p>
			<p>
				<?php $tf_profile_url = sprintf( '<a href="%1$s" target="_blank">%1$s</a>',
					esc_url( 'https://themeforest.net/user/aislin' ) ); ?>
				<?php printf( esc_html__( 'Item Support can be accessed from %s and includes:', 'ed-school'), $tf_profile_url ); ?>
			</p>
			<ul>
				<li><?php esc_html_e( 'Availability of the author to answer questions', 'ed-school' ); ?></li>
				<li><?php esc_html_e( 'Answering technical questions about item features', 'ed-school' ); ?></li>
				<li><?php esc_html_e( 'Assistance with reported bugs and issues', 'ed-school' ); ?></li>
				<li><?php esc_html_e( 'Help with bundled 3rd party plugins', 'ed-school' ); ?></li>
			</ul>

			<p><?php printf( esc_html__( 'Item Support %s include:', 'ed-school' ), sprintf( '<strong>%s</strong>', esc_html__( 'DOES NOT', 'ed-school') ) ); ?></p>
			<ul>
				<li><?php esc_html_e( 'Installation services', 'ed-school' ); ?></li>
				<li><?php esc_html_e( 'Customization services', 'ed-school' ); ?></li>
				<li><?php esc_html_e( 'Help and Support for non-bundled 3rd party plugins (i.e. plugins you install yourself later on)', 'ed-school' ); ?></li>
			</ul>
			<p><?php $tf_support_policy_url = sprintf( '<a href="%s" target="_blank">%s</a>',
					esc_url( 'http://themeforest.net/page/item_support_policy' ),
					esc_html__( 'Item Support Policy', 'ed-school' ) ); ?>
				<?php printf( esc_html__( 'More details about item support can be found in the ThemeForest %s.', 'ed-school'), $tf_support_policy_url ); ?></p>
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
		...
		public function envato_setup_ready() {
			...
			<a href="<?php echo esc_url( 'https://twitter.com/share' ); ?>" class="twitter-share-button"
			   ...
			   data-via="EnvatoMarket" data-size="large"><?php esc_html_e( 'Tweet', 'ed-school' ); ?></a>
			<?php 
			...
			<p><?php esc_html_e( 'Congratulations! The theme has been activated and your website is ready. Login to your WordPress
				dashboard to make changes and modify any of the default content to suit your needs.', 'ed-school'); ?></p>
			<p><?php $tf_downloads_url = sprintf( '<a href="%s" target="_blank">%s</a>',
					esc_url( 'http://themeforest.net/downloads' ),
					esc_html__( 'leave a 5-star rating', 'ed-school' ) ); ?>
				<?php printf( esc_html__( 'Please come back and %s if you are happy with this theme.', 'ed-school'), $tf_downloads_url ); ?></p>
			<p><?php $tweeter_url = sprintf( '<a href="%s" target="_blank">%s</a>',
					esc_url( 'https://twitter.com/aislinthemes' ),
					esc_html( '@aislinthemes' ) ); ?>
				<?php printf( esc_html__( 'Follow %s on Twitter to see updates. Thanks!', 'ed-school'), $tweeter_url ); ?></p>
				...
				<div class="envato-setup-next-steps-first">
					...
					<ul>
						...
						<a class="button button-primary button-large"
                            ...
                            target="_blank">
                            <?php esc_html_e( 'Follow @aislinthemes on Twitter', 'ed-school' ); ?></a>
						</li>
		...
		public function envato_market_admin_init() {
			...
			if ( ! empty( $_POST['oauth_session'] ) && ! empty( $_POST['bounce_nonce'] ) && wp_verify_nonce( $_POST['bounce_nonce'], 'envato_oauth_bounce_' . $this->envato_username ) ) {
				...
				if ( is_wp_error( $response ) ) {
					...
					if ( $result !== true ) {
						esc_html_e( 'Failed to get oAuth token. Please go back and try again', 'ed-school' );
						exit;
					}
				}
			}
				...
				if ( ! $option || empty( $option['oauth'] ) || empty( $option['oauth'][ $this->envato_username ] ) || empty( $envato_items ) || empty( $envato_items['items'] ) || ! envato_market()->items()->themes( 'purchased' ) ) {
					...
					if ( ! $dissmissed_time || $dissmissed_time < strtotime( '-7 days' ) ) {
						...
						<div class="notice notice-warning notice-dtbwp-themeupdates is-dismissible">
							<p><?php
								esc_html_e( 'Please activate ThemeForest updates to ensure you have the latest version of this theme.', 'ed-school' );
								?></p>

							<p>
								<a class="button button-primary" href="<?php echo esc_url( $this->get_oauth_login_url( admin_url( 'admin.php?page=' . envato_market()->get_slug() . '' ) ) ) ?>"><?php echo esc_html_e( 'Activate Updates', 'ed-school' ); ?></a>
							</p>
						</div>
						<?php 
		...
		public function render_oauth_login_description_callback() {
			printf( esc_html__( 'If you have purchased items from %s on ThemeForest or CodeCanyon please login here for quick and easy updates.', 'ed-school' ), esc_html( $this->envato_username ) );
		}
		...
		public function render_oauth_login_fields_callback() {
			...
			<div class="oauth-login" data-username="<?php echo esc_attr( $this->envato_username ); ?>">
				<a href="<?php echo esc_url( $this->get_oauth_login_url( admin_url( 'admin.php?page=' . envato_market()->get_slug() . '#settings' ) ) ); ?>"
				   class="oauth-login-button button button-primary"><?php esc_html_e( 'Login with Envato to activate updates', 'ed-school' ); ?></a>
			</div>
		<?php
		}
